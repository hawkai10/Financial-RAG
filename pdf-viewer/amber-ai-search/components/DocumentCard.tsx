...
const DocumentCard = forwardRef<HTMLDivElement, DocumentCardProps>(({ document, isHighlighted, onViewPdf }, ref) => {
  const { 
    sourceIcon, 
    sourceType, 
    sourcePath, 
  ...
  const isPreviewable = ['pdf', 'word', 'excel'].includes(fileType);
    ...
    <div ref={ref} className={`bg-white border rounded-lg p-4 transition-all duration-300 ${highlightClass}`}>
      ...
      <div className="mt-4 text-sm text-slate-600 pl-9 space-y-3">
        ...
        <div>
            ...
            <a
              href="#"
              className={`${isPreviewable ? 'ml-4' : ''} text-blue-600 hover:underline font-medium inline-flex items-center`}
